...
interface DialogueEditorConfigProps {
  projectId: string;
}

export function DialogueEditorConfig({ projectId }: DialogueEditorConfigProps) {
  const [enabled, setEnabled] = useState(true);
  const [mode, setMode] = useState<'single' | 'two-pass'>('single');
    ...
    async function loadSettings() {
      ...
      try {
        const [enabledResult, modeResult] = await Promise.all([
          projectSettingsApi.get(projectId, 'dialogue_editor_enabled'),
          projectSettingsApi.get(projectId, 'dialogue_editor_mode'),
        ]);
        if (cancelled) return;
        if (enabledResult.success && enabledResult.data !== null && enabledResult.data !== undefined) {
          setEnabled(Boolean(enabledResult.data));
        }
        if (modeResult.success && modeResult.data === 'two-pass') {
          setMode('two-pass');
        } else {
          setMode('single');
        }
  ...
  const handleToggleEnabled = useCallback(async () => {
    const next = !enabled;
    setEnabled(next);
    await projectSettingsApi.set(projectId, 'dialogue_editor_enabled', next);
  }, [enabled, projectId]);

  const handleModeChange = useCallback(async (nextMode: 'single' | 'two-pass') => {
    setMode(nextMode);
    await projectSettingsApi.set(projectId, 'dialogue_editor_mode', nextMode);
  }, [projectId]);
        ...
        <div style={{ paddingTop: 12 }}>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            <ModeButton
              ...
              label={zhTW.dialogueEditor.modeSingle}
            />
            <ModeButton
              selected={mode === 'two-pass'}
              onClick={() => handleModeChange('two-pass')}
              label={zhTW.dialogueEditor.modeTwoPass}
            />
          </div>
        </div>
      )}
